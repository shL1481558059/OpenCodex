...
11 个 apply_patch 测试断言更新为透传行为（name 保持原样、arguments 为原始 JSON、无 exec_command/heredoc）。159 个测试全通过。

### 遗留

- `ApplyPatchTools.cs` 中 `ExpandApplyPatchProxyTools` / `RebuildApplyPatchGrammar` / `ApplyPatchExecCommand` 等方法不再被调用，属死代码，可单独清理
- chat 通道流式循环里 `IsApplyPatchToolName` 的 skip 逻辑仍存在，导致 apply_patch 工具调用在 chat 通道仍无 delta（延迟到流末），后续可清理
