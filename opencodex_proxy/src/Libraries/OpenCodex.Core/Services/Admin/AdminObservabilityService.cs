...
public sealed class AdminObservabilityService : IAdminObservabilityService
{
    ...
    public AdminObservabilityService(IOpenCodexRuntimeSettingsProvider settingsProvider)
    {
        _settingsProvider = settingsProvider;
    }

    public ApiResult<LogsPageResponse> ReadLogsPage(
        object? page,
        object? pageSize,
        ...
        bool isSuperadmin)
    {
        return ApiResult.Success(LogsPageResponse.From(ReadLogsPage(
    ...
    public ApiResult<IReadOnlyDictionary<string, object>> ReadLogFilterOption(
        string field,
        object? query,
        IReadOnlyDictionary<string, object?> filters,
        string currentUsername,
        bool isSuperadmin)
    {
        return ApiResult.Success(ReadLogFilterOption(
            _settingsProvider.GetSettings(),
            field,
            query,
            ScopedFilters(filters, currentUsername, isSuperadmin)));
    ...
    public ApiResult<LogDetailResponse> ReadLogById(
        long logId,
        string currentUsername,
        bool isSuperadmin)
    {
        ...
        return log is null
            ? ApiResult.Fail<LogDetailResponse>(AdminObservabilityErrorCodes.NotFound, "log not found")
            : ApiResult.Success(LogDetailResponse.From(log));
    }

    public ApiResult<StatsResponse> ReadStats(
        string rangeKey,
        object? startTs,
        object? endTs,
        string currentUsername,
        bool isSuperadmin)
    {
        return ApiResult.Success(StatsResponse.From(ReadStats(
            _settingsProvider.GetSettings(),
            rangeKey,
            startTs,
            endTs,
        ...
        IReadOnlyDictionary<string, object?>? filters = null)
    {
        ...
        var events = query
            ...
            .Take(parsedPageSize)
            .AsEnumerable()
            .Select(EfServiceSupport.ToRequestLogEventDto)
            .ToList();
        ...
        IReadOnlyDictionary<string, object?>? filters = null)
    {
        ...
        if (!EfServiceSupport.TryConvertInt64(logId, out var parsedId))
        {
            return null;
        }
        ...
        return log is null ? null : EfServiceSupport.ToRequestLogDto(log);
    }
        ...
        string? ownerUsername = null)
    {
        ...
        for (var index = 0; index < bucketCount; index++)
        {
            ...
            points.Add(new StatsPointDto(
                EfServiceSupport.TimestampToIso(bucketEnd),
                Math.Round(cost, 6),
                inputTokens,
                cachedTokens,
                outputTokens,
        ...
        return new StatsDto(
            resolved.RangeKey,
            EfServiceSupport.TimestampToIso(resolved.StartTs),
            EfServiceSupport.TimestampToIso(resolved.EndTs),
            resolved.GranularityMinutes,
        ...
        IReadOnlyDictionary<string, object?> filters)
    {
        ...
        {
            if (EfServiceSupport.IsEmptyLogFilterValue(value))
            {
                continue;
            }
    ...
    {
        return EfServiceSupport.TryConvertInt32(value, out var parsed)
    ...
    {
        return EfServiceSupport.TryConvertInt64(value, out var parsed)
    ...
    {
        return EfServiceSupport.TryConvertDouble(value, out var parsed)
    ...
    {
        var queryText = (search?.ToString() ?? string.Empty).Trim();
        if (queryText.Length > 0 && !EfServiceSupport.TryConvertInt64(queryText, out _))
        {
            return [];
        }
        ...
        if (queryText.Length > 0 && EfServiceSupport.TryConvertInt64(queryText, out var parsed))
        {
    ...
    {
        return EfServiceSupport.TryConvertInt32(page, out var parsed)
            ? Math.Max(1, parsed)
            : 1;
    }
    ...
    {
        return EfServiceSupport.TryConvertInt32(pageSize, out var parsed)
        ...
        object? endTs)
    {
        ...
        {
            var parsedEnd = EfServiceSupport.ParseTimestamp(endTs);
            var parsedStart = EfServiceSupport.ParseTimestamp(startTs);
