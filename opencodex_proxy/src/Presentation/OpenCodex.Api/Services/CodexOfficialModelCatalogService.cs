...
public sealed class CodexOfficialModelCatalogService : ICodexOfficialModelCatalogService
{
    ...
    {
        var baseModels = _baseModels.Value;
        ...
        var template = baseModels.FirstOrDefault(model =>
                string.Equals(ReadString(model, "slug"), "gpt-5.5", StringComparison.OrdinalIgnoreCase))
            ?? baseModels.FirstOrDefault();
        if (template is null)
        {
            return result;
        }

        foreach (var routed in routedModels)
        {
            if (string.IsNullOrWhiteSpace(routed.Model) || !seen.Add(routed.Model))
            {
                continue;
            }

            var clone = Clone(template);
            clone["slug"] = routed.Model;
            clone["display_name"] = ResolveDisplayName(routed, catalogByModel);
            ApplyOneMillionContext(clone);
            result.Add(clone);
        }
    ...
    private static string ResolveDisplayName(
        ProxyModelCapabilityDto routed,
        IReadOnlyDictionary<string, ModelInfoResponse> catalogByModel)
    {
        if (catalogByModel.TryGetValue(routed.Model, out var info)
