...
public sealed class ProxyCompatibilityTests : IClassFixture<OpenCodexApiFactory>
{
    ...
    public async Task WebSearchStream_ChatUpstream_PreservesNativeToolSearchCall()
    {
        ...
        // 上游先返回 web_search 调用（触发 simulate 循环），再返回 tool_search 调用（非 web 工具，终止循环）
        var upstream = new RecordingStreamUpstreamClient(
        ...
        // 流式事件中 tool_search 应输出为 tool_search_call，而非 function_call
        var body = string.Concat(events);
        Assert.Contains("\"type\":\"tool_search_call\"", body, StringComparison.Ordinal);
        Assert.Contains("\"name\":\"tool_search\"", body, StringComparison.Ordinal);
        Assert.DoesNotContain("\"type\":\"function_call\"", body, StringComparison.Ordinal);

        // 最终 ResponsePayload 也应包含 tool_search_call
        Assert.NotNull(streamResult.ResponsePayload);
        var output = Assert.IsType<List<object?>>(streamResult.ResponsePayload!["output"]);
        Assert.Contains(output, item =>
