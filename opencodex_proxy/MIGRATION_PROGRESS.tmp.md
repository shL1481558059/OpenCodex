...
- Installed .NET SDK 10.0.300 into `~/.dotnet`.
...
Local smoke: `dotnet run` without `OPENCODEX_ADMIN_STATIC_PATH` served `path/to/repo/frontend/dist/admin/index.html` for `GET /admin`.
...
- `global.json` only selects the SDK when the .NET 10 SDK is reachable; local commands still need `~/.dotnet` first in `PATH` unless the shell profile is updated.
...
- The default shell `dotnet` still resolves to system SDK 9.0.305 unless `~/.dotnet` is placed first in `PATH`.
...
- Future .NET migration work should run from `path/to/repo/opencodex_proxy`.
...
- Continue using `path/to/repo/opencodex_proxy` as the .NET project root.
...
- Use `path/to/Ylg/Ylg.WebPortal/Libraries/Ylg.Core` as the reference implementation.
...
- `path/to/Ylg/Ylg.WebPortal/Libraries/Ylg.Core/Ylg.Core.csproj`
- `path/to/Ylg/Ylg.WebPortal/Libraries/Ylg.Core.Base/Data/IRepository.cs`
- `path/to/Ylg/Ylg.WebPortal/Libraries/Ylg.Data/EfRepository.cs`
- `path/to/Ylg/Ylg.WebPortal/Libraries/Ylg.Core.Domain/Domain/BaseEntity.cs`
- `path/to/Ylg/Ylg.WebPortal/Libraries/Ylg.Web.Framework/Infrastructure/DependencyRegistrar.cs`
- `REPOSITORY_GUIDE.md`
- `CLASS_LIBRARY_GUIDE.md`
- `PROJECT_ARCHITECTURE.md`
...
- Use `path/to/Ylg/Ylg.WebPortal/Libraries/Ylg.Core` as the reference for class-library layering.
- Continue checking the surrounding class-library architecture documents before the next migration unit.

Reference observations:

- `path/to/Ylg/Ylg.WebPortal/Libraries/Ylg.Core.Base/Data/IRepository.cs` defines a generic `IRepository<TEntity>` constrained to domain entities.
- `path/to/Ylg/Ylg.WebPortal/Libraries/Ylg.Data/EfRepository.cs` implements the generic repository.
- `path/to/Ylg/Ylg.WebPortal/Libraries/Ylg.Web.Framework/Infrastructure/DependencyRegistrar.cs` registers the open generic implementation:
  - `RegisterGeneric(typeof(EfRepository<>)).As(typeof(IRepository<>))`
- `path/to/Ylg/Ylg.WebPortal/Libraries/Ylg.Core.Domain/Domain/BaseEntity.cs` is the domain entity base model.
...
- This rule was added to `path/to/repo/AGENTS.md`.
