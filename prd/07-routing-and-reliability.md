...
- 无 `Retry-After` 时按 `2s * 2^attempt`，指数项最大 8 秒。
- 所有路径叠加 0 到 20% 的向上抖动，并保证任何重试前至少等待 2 秒。
...
2. 无该头时指数项不超过 8 秒，叠加抖动后不超过 9.6 秒。
3. 任何重试路径都不得零间隔重发，最小间隔 2 秒。
4. jitter 已实现，并通过可注入退避在 `UpstreamRetryBackoffTests` 中断言等待序列。
