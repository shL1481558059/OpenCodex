...
| 项目 | 说明 |
|---|---|
| 触发入口 | 模型信息页工具栏"导出"按钮 + "导入"下拉（同步最新模型 / 导入本地 json / 覆盖已有模型） |
| 权限 | 仅超级管理员 |
...
| 项目 | 说明 |
|---|---|
| 触发入口 | "导入"下拉 -> `同步最新模型` / `覆盖已有模型` |
| 权限 | 仅超级管理员 |
| 同步地址 | 环境变量 `OPENCODEX_MODEL_CATALOG_SYNC_URL`（空白回落内置默认地址），管理台不可改、不展示 |
| 增量同步 | 远端有、本地无的模型创建（`source = sync`）；本地已存在的模型完全不动（含停用模型不复活）；已存在供应商不动 |
| 覆盖已有模型 | 在增量基础上额外改写已存在模型的名称、匹配规则、能力与价格（`source = sync`）；启用状态、供应商信息、本地独有模型不受影响；远端 `pricing: null` 不删除本地价格 |
| 拉取硬化 | 60 秒超时、5 MB 上限、最多 3 次重定向、自动解压、剥 BOM；仅允许 http/https |
| 失败日志 | warning 一行，含地址、HTTP 状态、失败原因与完整响应体（不截断） |
| 预检流程 | POST `/model-catalog/sync?mode=incremental\|overwrite&dryRun=true` -> 对话框展示差异 -> 确认 -> POST `dryRun=false` 重新拉取并写入 |
| 覆盖确认 | 危险提示 + 勾选「我已了解本地修改将被覆盖」+ 可先导出当前目录作为快照 |
