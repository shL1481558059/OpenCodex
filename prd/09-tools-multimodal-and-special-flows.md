...
- 混合调用时不提前续轮；通过数据库持久化结果恢复客户端回传历史。
- 续传结果不设置独立 TTL，也不运行后台清理；超级管理员清除全部日志时必须在同一事务中删除全部 Web Search 续传记录。
- 续传记录缺失但客户端回传完整结果时自动补写数据库；完全缺失时生成明确不可用的工具结果并继续会话，不自动重新搜索。
...
1. 主路由不感知请求是否含图片；候选按亲和、优先级、活跃请求数和原始顺序排序（原先传给路由服务却从未被读取的 `requestContainsImages` 参数已删除）；
2. 已选候选命中显式模型映射且 `SupportsImage=true` 时，图片直接交给该候选；
3. 已选候选命中显式模型映射但不支持图片时，才触发 OCR/视觉描述降级；
4. 降级只使用该 owner 显式配置的视觉路由，按主、兜底顺序尝试；自动发现规则已整体移除；
5. 视觉路由只来自该 owner 的显式配置（主 + 可选兜底），没有配置行时 OCR 路径返回 400：`vision transfer model is not configured for owner '<owner>'`；配置存在但主与兜底都失效时返回 400：`configured vision transfer route is unavailable: <reason>`；
...
flowchart TD
    ...
    B -->|是| C["直接走视觉模型"]
    B -->|否| D{"该 owner 配置了视觉转移模型?"}
    D -->|否| E["返回 400 未配置或配置失效"]
    D -->|是| F["按主、兜底顺序检查 OCR 缓存"]
    F -->|命中| G["读取文本与描述"]
    F -->|未命中| H["调用视觉 OCR 子请求"]
    H --> I["写 OCR 子日志并缓存;失败则转兜底"]
    I --> G
    G --> J["重写主请求文本"]
    J --> K["继续路由和协议转换"]
```
...
- OCR 文件缓存是进程文件系统级共享缓存：Data URL 按解码字节哈希，远程图片按 URL 字符串哈希，并把视觉渠道 id 与上游模型名并入缓存键；缓存 Key 仍不包含 Owner 与访问 Key，命中后会跨用户复用识别结果，属于租户隔离风险；
