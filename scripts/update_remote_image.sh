...
ssh "${SSH_OPTS[@]}" "$SSH_TARGET" \
  "REMOTE_DEPLOY_DIR='$REMOTE_DEPLOY_DIR' IMAGE_NAME='$IMAGE_NAME' SERVICE_NAME='$SERVICE_NAME' OLD_SERVICE_NAMES='$OLD_SERVICE_NAMES' HOST_BIND='$HOST_BIND' HOST_PORT='$HOST_PORT' CONTAINER_PORT='$CONTAINER_PORT' DOCKER_PLATFORM='$DOCKER_PLATFORM' CONTAINER_TIMEZONE='$CONTAINER_TIMEZONE' CONTAINER_MEMORY_LIMIT='$CONTAINER_MEMORY_LIMIT' bash -s" <<'REMOTE_SCRIPT'
set -euo pipefail
